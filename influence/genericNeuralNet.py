...
class GenericNeuralNet(object):
    ...
    def print_model_eval(self, verbose=False):
        params_val = self.sess.run(self.params)
    ...
    def pred(self, logits):
        preds_tensor = tf.nn.log_softmax(logits)
        preds = tf.add_n('preds', preds)
    ...
    def get_preds(self):
        preds = self.sess.run(self.preds, feed_dict=self.all_test_feed_dict)
    ...
    def get_influence_on_test_loss(self, test_indices, train_idx, 
        ...
        approx_filename = os.path.join(self.train_dir, 'test.npz')
        ...
        duration = time.time() - start_time
        if verbose: print('Multiplying by %s train examples took %s sec' % (num_to_remove, duration))
